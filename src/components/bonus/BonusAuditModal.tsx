...
interface BonusAuditModalProps {
  isOpen: boolean;
  onClose: () => void;
  bonus: BonusItem | null;
}

export function BonusAuditModal({
  isOpen,
  onClose,
  bonus,
}: BonusAuditModalProps) {
  const [logs, setLogs] = useState<AuditLogItem[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (!isOpen || !bonus) return;

    setLoading(true);
    setError(null);

    fetch(`/api/v1/bonuses/${bonus.id}/audit`)
      .then((r) => r.json())
      .then((json) => {
  ...
  function getActionLabel(action: string) {
    switch (action) {
      case 'CREATE_BONUS_PROPOSAL':
        return { label: 'Tạo Đề Xuất Thưởng', color: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/30' };
      case 'UPDATE_BONUS_BEFORE_APPROVAL':
        return { label: 'Chỉnh Sửa Trước Duyệt', color: 'text-blue-400 bg-blue-500/10 border-blue-500/30' };
      case 'APPROVE_BONUS':
        return { label: 'Phê Duyệt Chính Thức', color: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/30' };
      case 'REJECT_BONUS':
        return { label: 'Từ Chối Khoản Thưởng', color: 'text-red-400 bg-red-500/10 border-red-500/30' };
      default:
      ...
      <Card className="relative w-full max-w-2xl border-slate-700 bg-slate-900 shadow-2xl my-8">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">Nhật Ký Kiểm Toán Tài Chính (Audit Trail)</h2>
              <p className="text-xs text-slate-400">
                Lịch sử bất biến ghi nhận mọi biến động tài chính của khoản thưởng
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {loading ? (
            <div className="py-12 text-center text-xs text-slate-400 flex flex-col items-center gap-2">
              ...
              <span>Đang tải lịch sử kiểm toán...</span>
            </div>
          ) : logs.length === 0 ? (
              ...
              {logs.map((log) => {
                ...
                return (
                  <div key={log.id} className="relative group">
                    {/* Bullet */}
                    <div className="absolute -left-6 top-1 h-3 w-3 rounded-full border-2 border-slate-700 bg-slate-900 group-hover:border-blue-500 transition-colors" />
